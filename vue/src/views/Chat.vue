                ...
                <el-card style="width: 300px; height: 300px; color: #333">
                    ...
                    <div style="padding: 10px 0" v-for="user in users" :key="user.name">
                        ...
                        <span style="font-size: 12px;color: limegreen; margin-left: 5px"
                              v-if="user.name === chatUser">Chatting...</span>
                    </div>
                </el-card>
            </el-col>
            <el-col :span="20">
        ...
        methods: {
            ...
            createContent(remoteUser, nowUser, text) {  // 这个方法是用来将 json的聊天消息数据转换成 html的。
                ...
                if (nowUser) { // nowUser 表示是否显示当前用户发送的聊天消息，绿色气泡
                    html = "<div class=\"el-row\" style=\"padding: 5px 0\">\n" +
                        ...
                        // "    <img src='" + this.user.avatarUrl + "' style=\"object-fit: cover;\">\n" +
                        "  </span>\n" +
                        "  </div>\n" +
                        "</div>";
            ...
            init() {
                ...
                if (typeof (WebSocket) == "undefined") {
                    ...
                    console.log("您的浏览器支持WebSocket");
                    let socketUrl = "ws://localhost:8800/imserver/" + name;
                    if (socket != null) {
                        socket.close();
                        socket = null;
                    }
                    ...
                    socket.onmessage = function (msg) {
                        ...
                        if (data.users) {  // 获取在线人员信息
                            _this.users = data.users.filter(user => user.name !== name)  // 获取当前连接的所有用户信息，并且排除自身，自己不会出现在自己的聊天列表里
                        } else {
